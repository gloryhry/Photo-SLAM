      ...
      training_report_interval_(0)
{
    ...
    {
    case Lidar_SLAM::System::MONOCULAR:
    case Lidar_SLAM::System::IMU_MONOCULAR:
    {
        this->sensor_type_ = MONOCULAR;
    }
    break;
    case Lidar_SLAM::System::STEREO:
    case Lidar_SLAM::System::IMU_STEREO:
    {
    ...
    break;
    case Lidar_SLAM::System::RGBD:
    case Lidar_SLAM::System::IMU_RGBD:
    {
    ...
    for (auto& SLAM_camera : vpCameras) {
        ...
        //根据相机的类型获取相机的内参矩阵
        if (SLAM_camera->GetType() == Lidar_SLAM::GeometricCamera::CAM_PINHOLE) {
        ...
        else if (SLAM_camera->GetType() == Lidar_SLAM::GeometricCamera::CAM_FISHEYE) {
...
void GaussianMapper::run()
{
    ...
    while (!isStopped()) {
        ...
        if (hasMetInitialMappingConditions()) {//检查是否满足了初始化建图的条件（要求orbslam没关闭不为空且关键帧大于一定的数目）
            ...
            std::vector<Lidar_SLAM::KeyFrame*> vpKFs;
            std::vector<Lidar_SLAM::MapPoint*> vpMPs;
            {
                ...
                for (const auto& pKF : vpKFs){
                    ...
                    // 函数将存储关键点像素坐标和局部坐标的指针转移给新的关键帧 new_kf 对象的成员变量 kps_pixel_ 和 kps_point_local_，这样做是为了避免数据的复制，提高效率。
...
void GaussianMapper::combineMappingOperations()
{
    ...
    while (pSLAM_->getAtlas()->hasMappingOperation()) {
        Lidar_SLAM::MappingOperation opr =
            pSLAM_->getAtlas()->getAndPopMappingOperation();

        switch (opr.meOperationType)
        {
        case Lidar_SLAM::MappingOperation::OprType::LocalMappingBA:
        {
        ...
        case Lidar_SLAM::MappingOperation::OprType::LoopClosingBA:
        {
...
// keyframesToJson(result_dir_ / (std::to_string(getIteration()) + "_0_before_loop_correction"));
        ...
        case Lidar_SLAM::MappingOperation::OprType::ScaleRefinement:
        {
